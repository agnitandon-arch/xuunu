...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/user-credentials", async (req, res) => {
    try {
      ...
      res.json({
        ...
        // Fitbit
        hasFitbitCredentials: !!credentials.fitbitClientId && !!credentials.fitbitClientSecret,
        fitbitConnected: !!credentials.fitbitAccessToken,
        fitbitUserId: credentials.fitbitUserId || null,
  ...
  app.post("/api/fitbit/credentials", async (req, res) => {
    try {
      const { userId, clientId, clientSecret } = req.body;

      if (!userId || !clientId || !clientSecret) {
        return res.status(400).json({ error: "userId, clientId, and clientSecret are required" });
      }

      const existing = await storage.getUserApiCredentials(userId);
      const credentials = await storage.upsertUserApiCredentials({
        ...existing,
        userId,
      ...
      res.json({
        success: true,
        hasFitbitCredentials: !!credentials.fitbitClientId,
      });
    } catch (error) {
      console.error("Error saving Fitbit credentials:", error);
      res.status(500).json({ error: "Failed to save Fitbit credentials" });
    }
  });

  app.get("/api/fitbit/auth-url", async (req, res) => {
    try {
      const userId = req.query.userId as string;
      ...
      if (!credentials?.fitbitClientId) {
        return res.status(400).json({ 
          error: "Fitbit API credentials not configured. Please add your Client ID and Secret first." 
        });
      }
  ...
  app.get("/api/fitbit/callback", async (req, res) => {
    try {
      ...
      const stateData = fitbitOAuthStates.get(state as string);
      if (!stateData) {
      ...
      if (!credentials?.fitbitClientId || !credentials?.fitbitClientSecret) {
        return res.status(400).send("Fitbit credentials not found");
      }

      const basicAuth = Buffer.from(`${credentials.fitbitClientId}:${credentials.fitbitClientSecret}`).toString('base64');
      const host = req.get('host') || 'localhost:5000';
      const protocol = host.includes('replit') ? 'https' : req.protocol;
      const redirectUri = `${protocol}://${host}/api/fitbit/callback`;
      ...
      await storage.upsertUserApiCredentials({
        ...credentials,
        userId,
        ...
        <html>
          ...
          <body style="font-family: system-ui; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #000; color: #fff;">
            <div style="text-align: center;">
              <h1 style="color: #0066FF;">Fitbit Connected!</h1>
      ...
      res.status(500).send("Failed to complete Fitbit authorization");
    }
  });

  app.post("/api/fitbit/refresh-token", async (req, res) => {
    try {
      const { userId } = req.body;
      ...
      if (!credentials?.fitbitRefreshToken || !credentials?.fitbitClientId || !credentials?.fitbitClientSecret) {
        return res.status(400).json({ error: "Fitbit not connected or credentials missing" });
      }
      ...
      const tokenResponse = await fetch("https://api.fitbit.com/oauth2/token", {
        ...
        body: new URLSearchParams({
          ...
          refresh_token: credentials.fitbitRefreshToken,
        }),
      });
  ...
  app.get("/api/fitbit/data", async (req, res) => {
    try {
      const userId = req.query.userId as string;
      const dataType = req.query.type as string || "activities";
      ...
      if (!credentials?.fitbitAccessToken) {
        return res.status(400).json({ error: "Fitbit not connected" });
      }

      if (credentials.fitbitTokenExpiresAt && new Date(credentials.fitbitTokenExpiresAt) < new Date()) {
      ...
      if (!response.ok) {
        ...
        return res.status(response.status).json({ error: "Failed to fetch Fitbit data" });
      }
  ...
  app.get("/api/fitbit/status", async (req, res) => {
    try {
      ...
      res.json({
        hasCredentials: !!credentials?.fitbitClientId && !!credentials?.fitbitClientSecret,
        isConnected: !!credentials?.fitbitAccessToken,
        fitbitUserId: credentials?.fitbitUserId || null,
        tokenExpired: credentials?.fitbitTokenExpiresAt 
          ? new Date(credentials.fitbitTokenExpiresAt) < new Date() 
          : false,
      });
    } catch (error) {
      console.error("Error checking Fitbit status:", error);
      res.status(500).json({ error: "Failed to check Fitbit status" });
    }
  });
